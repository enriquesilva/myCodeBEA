...
class PuntocontrolsController extends AppController {
	...
	public function index() {
        ...
        if (!empty($this->data)) {
		  ...
		  if(empty($resultado)){
			  ...
			  $this->set('prevRuta',$this->data['puntocontrol']['ruta_id']);
		  }
		  else{
    ...
    public function add($id = null) {
        ...
        $this->loadModel('Ruta');
        $this->loadModel('Puntocontrol');
        $resultado = $this->Puntocontrol->find('all',array('conditions' => array('Puntocontrol.ruta_id' => $id)));
		$this->set('resultado',$resultado);
        
        if($this->Session->check('LastPoint.prRuta')){ $this->set('rutas', $this->Ruta->find('list', array('fields' => array('Ruta.id', 'Ruta.descripcion'),'order' => array('Ruta.descripcion ASC'),'conditions'=>array('Ruta.id'=>$this->Session->read('LastPoint.prRuta'))))); }
		else{$this->set('rutas', $this->Ruta->find('list', array('fields' => array('Ruta.id', 'Ruta.descripcion'),'order' => array('Ruta.descripcion ASC'),'conditions'=>array('Ruta.id <>'=>0,'Ruta.id'=>$id))));}
    ...
    public function edit($id = null) {
	    ...
	    $resultado = $this->Puntocontrol->find('all',array('conditions' => array('Puntocontrol.ruta_id' => $r)));
		$this->set('resultado',$resultado);
	    $this->loadModel('Ruta');
        $this->set('rutas', $this->Ruta->find(
        	'list', array('fields' => array('Ruta.id', 'Ruta.descripcion'),
