...
<script type="text/javascript">	
	...
	$(function(){
		...
		function calcular(hMenor,hMayor){
			...
			return difHours+":"+difMins+":"+difSecs;			
		} 
		...
		$('.header').click(function(event){  
			...
			$('tr').each(function(){				
				if(i==0){
					var value= $('td:nth-child('+ix+')').text();
					console.log("Una vez"); 
					console.log(value);  					
					hora1= value;
					i++;
				}		
				var hora2= $('td:nth-child('+ix+')').text();
				console.log("hora2");
				console.log(hora2);
				var horaTotal= calcular(hora1, hora2);
				console.log("horaTotal");
				console.log(horaTotal);
				hora1=hora2;
				//console.log("Diferencia de tiempo");  
				//console.log(horaTotal);  
				$('td:nth-child('+ix+') .dif').show(); 
			});
		}); 
	});
</script>
	  ...
	  <tbody>
	  	...
	  	foreach($resultado['Detalles'] as $d) { 
	  		echo "<tr>"; 
	  			...
	  			for($c2 = 0; $c2 < $c; $c2++){
					if(isset($d[$idpc[$c2]]) && !empty($d[$idpc[$c2]])){ echo "<td>".$d[$idpc[$c2]]." <span class='dif'>(02:05)</span></td>";}
					else{ echo "<td class='tdError'>NO PASÓ <span class='dif'>(0)</span></td>";}
	  			}
	  		echo "</tr>";
	  		$c2++;
		}
